...
description: Run actions when records change or on a schedule — set fields, create records, send email, call an API — with a click or a trigger.
sidebar:
  order: 6
---
...
> workspace-wide feature with no rule involved. See the [webhooks guide](/guides/webhooks/).
...
- **Same expression language as [formula fields](/concepts/formulas/)** — same parser, same
  operators, same rules about what an empty value does.
- **Only number targets compute.** A text field still does literal substitution, so
  `{Name} - 1` on a text field writes exactly what it always wrote.
...
lists what is allowed. **Email and API calls are not available on a webhook-triggered rule** — have
the webhook create a record, and let a record-created rule do the outbound work.
...
secret. Paste the URL into whatever should trigger it — Typeform's *Connect → Webhooks*, or a
Zapier/Make "Webhooks" action. Both the URL and the secret have a **Copy** button, and
**Regenerate** mints a new pair (the old one stops working immediately).

`{payload.…}` tokens read the delivered body in any action field: `{payload.email}`, or
`{payload.answers.0.value}` for something nested. The panel shows the **last received payload** to
find the path you need.

Signing is optional: sign requests with the secret (`X-StoryOS-Signature: sha256=…` plus
`X-StoryOS-Timestamp`) and StoryOS verifies them. Unsigned requests are still accepted, which is
what lets a no-code tool call the endpoint with nothing but a URL.
...
- **External actions are queued, not inline.** Email, API calls and agent runs are handed to a
  ...
  with backoff instead of stalling the write or firing twice on a retry.
...
- **Run log**: every execution (ok / error / skipped / skipped_quota) with duration is kept for 90
  days, queryable across the whole workspace at `/w/:ws/runs`.
...
items, far below anything that would strain an instance. A job that needs hundreds of steps wants
**create many records** or a scheduled rule over a filter, not a self-triggering countdown.
