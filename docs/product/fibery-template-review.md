# Fibery template library — full review (2026-07-10)

Founder directive: review Fibery's template library, decide what StoryOS needs, and max out what
our machinery can already deliver. Source: fibery.com/templates + the 13 pages dug individually.
...
One meta-lesson from Fibery's library beats any single template: **every template ships with a
...
Fibery: `Meeting ↔ Action Item`; "capture meeting notes (daily, project status), track action
...
Fibery: `Article ↔ Topic`, idea capture → rating → draft → publish, writer-workload views.
...
Fibery ships NINE databases (Journey, Stage, Action, Emotion, Goal, Opportunity, Pain point,
...
Fibery: `Event, Expense, Task, Category`; "track budget, see which category is most expensive."
...
Fibery: `Video, Expense, Stage, Task` + script writing. Ours: Videos (Stage select
...
Fibery: `Calendar moment, Content, Platform` — the interesting idea is **Calendar Moments**
...
Two Fibery templates, ~70% overlapping (`Campaign, Content, Audience, Key metric / Channel,
...
Fibery: `Account, Opportunity, Sales task, Contact` — a real pipeline. Our agency-crm (Leads +
...
Fibery: `Team ↔ Employee` + manager self-relation + a dedicated **org-chart view** + employee map.
...
Fibery's 8-database version (Country, Year, Allocation, Overtime, Public Holiday…) with computed
...
Fibery: one-way sync of Repository/Branch/Issue/PR/Member; value = linking PRs to work items.
...
auto-links the PR to record MN-123 (Fibery makes you link manually); (c) self-hosted = your token
...
For Fibery this is a sync integration (User/Issue/Document/Initiative/Project/Milestone/Team/
